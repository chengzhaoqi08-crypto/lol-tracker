...
拉最近 10 场对局(**全模式**:排位 / 匹配 / 大乱斗 / 斗魂竞技场;真实模式走 Match-V5,demo 模式按段位生成),加权求分:

```
分数 = 45×胜率 + 35×min(场均KDA/5,1) + 20×参团率
```

> 跨模式公平:大乱斗 / 斗魂竞技场没有补刀概念,故评分**不计补刀**(补刀/分仍作信息展示)。
> 斗魂竞技场没有传统胜负,用名次(前 4 名算赢)。每场都标注模式,评分卡含模式分布。
> 区域路由:Account-V1 用 americas/asia/europe;**Match-V5 的 OCE/东南亚走 `sea`**(与账号路由不同)。
